// Inline movement test - copy of applyMovement function for isolation

function applyMovement(state, input, movement, dt) {
...
function runAccelerationTest() {
  ...
  for (let tick = 0; tick < tickCount; tick++) {
    ...
    measurements.samples.push({ time, speed, tick })
    ...
    if (measurements.time50 === null && speed >= TEST_CONFIG.maxSpeed * 0.5) {
      ...
      console.log(`MARKER: 50% speed (${speed.toFixed(3)} m/s) reached at ${time.toFixed(3)}s`)
    }

    if (measurements.time80 === null && speed >= TEST_CONFIG.maxSpeed * 0.8) {
      measurements.time80 = time
      console.log(`MARKER: 80% speed (${speed.toFixed(3)} m/s) reached at ${time.toFixed(3)}s`)
    }

    if (measurements.time90 === null && speed >= TEST_CONFIG.maxSpeed * 0.9) {
      measurements.time90 = time
      console.log(`MARKER: 90% speed (${speed.toFixed(3)} m/s) reached at ${time.toFixed(3)}s`)
    }
  ...
  console.log(`\nMAXSPEED: Max speed reached: ${measurements.maxReached.toFixed(3)} m/s at ${measurements.maxTime.toFixed(3)}s`)
...
function runDecelerationTest() {
  ...
  let time = 0
  const measurements = { timeToStop: null, samples: [] }
  const tickCount = 60
  ...
  for (let tick = 0; tick < tickCount; tick++) {
    ...
    if (measurements.timeToStop === null && speed < 0.05) {
      ...
      console.log(`MARKER: Stopped at ${time.toFixed(3)}s`)
    }
...
function runAirStrafeTest() {
  ...
  for (let tick = 0; tick < tickCount; tick++) {
    ...
    samples.push({ time, horizSpeed, vy: state.velocity[1], tick })
  ...
  console.log(`\nMAXAIRSPEED: Max horizontal speed while airborne: ${maxAirSpeed.toFixed(3)} m/s at ${maxAirTime.toFixed(3)}s`)

  return { maxAirSpeed, maxAirTime, samples }
}
...
function validateResults(accelMeasurements, decelMeasurements, strafeMeasurements) {
  ...
  const checks = [
    ...
    {
      name: 'Air strafe builds lateral velocity > 0.5 m/s',
